...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        if x not in self.is_spot_free:
            bounds = lambda i : self.statespace_lo[i] <= x[i] <= self.statespace_hi[i]
            self.is_spot_free[x] = bounds(0) and bounds(1) and self.occupancy.is_free(x)
        return self.is_spot_free[x]
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        # can be python-ized to one line but no need
        for x_adj, y_adj in adjacent:
            state = (x[0] + x_adj * self.resolution, x[1] + y_adj * self.resolution)
            state = self.snap_to_grid(state)
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            for x_neigh in self.get_neighbors(x_current):
                ...
                self.est_cost_through[x_neigh] = tentative_cost_to_arrive + self.distance(x_current, self.x_goal)
        return False
        ########## Code ends here ##########
