...
class PoseController:
    ...
    def __init__(self, k1, k2, k3, V_max=0.5, om_max=1):
        ...
        #rospy.init_node('orientation_params', anonymous=True)
    ...
    def compute_control(self, x, y, th, t):
        ...
        Hints: You'll need to use the wrdapToPi function. The np.sinc function
        ...
        rh = Float64()
        delt = Float64()
        rho = ((self.y_g - y) ** 2 + (self.x_g - x) ** 2) ** .5
        alpha = wrapToPi(np.arctan2((self.y_g - y), (self.x_g - x)) - th)
        delta = wrapToPi(np.arctan2((self.y_g - y), (self.x_g - x)) - self.th_g)
        alph.data = alpha
        rh.data = rho
        delt.data = delta
        self.alpha_pub.publish(alph)
        self.rho_pub.publish(rh)
        self.delta_pub.publish(delt)
